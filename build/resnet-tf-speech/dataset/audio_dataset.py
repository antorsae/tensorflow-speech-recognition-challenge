...
def collate(batch):
    ...
    tensors_mel  = torch.stack([batch[b][0]for b in range(batch_size)], 0)
    tensors_mfcc = torch.stack([batch[b][1]for b in range(batch_size)], 0)
    if batch[0][2] is None:
        labels = None
    else:
        labels = torch.from_numpy(np.array([batch[b][2]for b in range(batch_size)])).long()
    return [tensors_mel, tensors_mfcc, labels,indices]


# #data iterator ----------------------------------------------------------------
class AudioDataset(Dataset):

    def __init__(self, split,  unknowns_csv = None, transform=None, mode='train'):
        ...
        if unknowns_csv is not None:
            lines_unknown = read_list_from_file(unknowns_csv)
            lines_unknown = ['test/audio/' + s for s in lines_unknown ]
            lines.extend(lines_unknown)
